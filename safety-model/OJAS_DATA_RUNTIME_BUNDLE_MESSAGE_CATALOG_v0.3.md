# Ojas Data Runtime Bundle — Message Catalog v0.3
...
| Date | 2026-05-23 |
| Owner | SPHUTA / Ojas |
| Supersedes | Ojas Data CDDL v0.2 (encoding-specific; superseded by language-neutral catalog) |
| Applies | Seven v0.3 deltas from prior review + bundle-scope additions |
...
| `policy-state-digest` | optional | digest | Active policy/freeze state at message time |
| `instruction-context-digest` | optional | digest | Active instruction context (for I-1 enforcement evidence) |
...
| `input` | conditional | structured input shape per operation | **v0.3 delta:** constrained per operation type, not open `any` map |
...
**v0.3 delta on `input`:** the v0.2 `input: { * tstr => any }` open surface is replaced with **discriminated per-operation input types**. For human/operator requests, the input shape is documented per operation. For LLM-driven requests, `extracted-intent` is the only path. The two paths cannot be combined in a single request.
...
| `audit` | required | data-operation-audit (§5.2) | Always present |
| `effective-decision` | required | effective-decision-code | What happened |
| `resolved-intent` | conditional | resolved-intent | Present when resolution occurred |
| `preflight` | conditional | mutation-preflight-result | Present for CUD operations |
...
| `sanitized-feedback` | conditional | sanitized-feedback | Present when decision != allowed |
| `audit` | required | data-operation-audit | Always present |
...
| Field | Required? | Type | Notes |
|---|---|---|---|
| `audit-id` | required | identifier | Unique within Evidence Store |
...
| `emitter` | required | constant value `ojas-data` | **v0.3 delta:** typed constant, not enum value. Cannot be spoofed across types. |
| `correlation-id` | required | correlation-id | |
| `entity` | required | identifier | |
| `operation` | required | data-operation-code | |
...
**v0.3 delta:** `fields-returned` is required (not optional) for post-execution read/export audits. Omission produces an invalid audit. This prevents the Replit-pattern defense from being silently weakened by missing field-level detail.
...
| Field | Required? | Type | Notes |
|---|---|---|---|
| `audit-id` | required | identifier | The Ojas-emitted audit this hook references |
...
Every record in Evidence Store carries this envelope.

| Field | Required? | Type | Notes |
|---|---|---|---|
| `audit-id` | required | identifier | Globally unique |
...
| `created-at-ms` | required | timestamp-ms | |
| `payload` | required | record-type-specific structure | The actual audit content |
