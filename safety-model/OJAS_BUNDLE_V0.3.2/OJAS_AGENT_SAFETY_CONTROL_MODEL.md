...
| Field | Value |
|---|---|
| Status | Design baseline / freeze candidate, included in this pack as a one-pass copy |
...
1. **Ojas Data Runtime Bundle V0.3.2 message catalog** (`OJAS_DATA_RUNTIME_BUNDLE_MESSAGE_CATALOG_V0.3.2.md`) — schema-language-neutral catalog covering the v1 bundle scope:
   ...
   This catalog refines the prior Ojas Data V0.2 CDDL work and accumulates corrections through three revisions. The current revision is **V0.3.2**, which supersedes V0.3.1 and V0.3 (both retained for lineage only). Catalog lineage: V0.2 CDDL → V0.3 (seven deltas) → V0.3.1 (five review corrections) → V0.3.2 (five open-item resolutions: O-1 through O-5). The catalog form is schema-language-neutral; the existing V0.2 contributes the field discipline, not the encoding. See catalog §10 for full delta lineage.
