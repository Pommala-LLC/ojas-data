# Ojas Data Runtime Bundle — Message Catalog v0.3.1

| Field | Value |
|---|---|
| Status | Review-correction draft, supersedes v0.3 |
...
| Date | 2026-05-23 |
| Owner | SPHUTA / Ojas |
| Supersedes | v0.3 of this catalog (preserved for lineage; this revision is correction-only) |
| Lineage | Ojas Data CDDL v0.2 → catalog v0.3 → catalog v0.3.1 |
| Applies | Seven v0.3 deltas (preserved, §10.1) + five v0.3.1 review-correction deltas (new, §10.2) |
...
### 2.13 Discriminated input types (v0.3.1 stubs)

**v0.3.1 delta — discriminated input types (D-C1):**

v0.3 §2.10 declared that `data-execute-request.input` is a "discriminated per-operation input type" but did not enumerate those types. v0.3.1 introduces the six stubs below. Full field models are **deferred to v0.3.2** — this revision is a correction pass, not a feature pass.
...
**Deferred to v0.3.2:** entity, fields, filter structure, pagination, projection limits, ordering. These will mirror the resolved-intent shape (§2.3) but for human/operator-authored requests rather than LLM-extracted intent.
...
**Deferred to v0.3.2:** entity, field-value bindings per registry, batch shape, idempotency keys, association references.
...
**Deferred to v0.3.2:** entity, target record reference (PK or unique-key), field updates, optimistic concurrency token, expected-version pattern.
...
**Deferred to v0.3.2:** entity, target record reference, soft-delete preference, cascade declaration (if soft-delete declines cascade).
...
**Deferred to v0.3.2:** entity, format declaration, destination class (file/object-store/stream), maximum row count, masking profile override (if elevated policy permits).
...
**Deferred to v0.3.2:** entity, group-by axes, aggregate functions (closed enum), having-style filters, result-cap.
...
**v0.3.2 (future, non-binding):** The stubs will be filled in. Catalog readers should treat additional fields appearing on these types as **forward-compatible** payload — v0.3.1 validators that allow extra fields are correct; validators that close the input shapes via `additionalProperties: false` should defer that hardening to v0.3.2.
...
| `terminal` | required | boolean | Whether the agent session is terminated or only this request blocked |
...
| `partition-key` | required | bounded string | Tenant-id by default for tenant isolation in stream |
...
### 10.3 What v0.3.1 explicitly does NOT do

To prevent scope creep — v0.3.1 is a correction pass, and the following are explicitly **not** in scope:

- Full field shapes for the six §2.13 input types (deferred to v0.3.2)
...
Items that emerged during catalog drafting and need decision before freeze:

**O-1.** Should `credential-handle-request` (§4.1) carry the requesting `entity` field for tighter per-entity scoping by the external authority? Trade-off: simpler scope hint vs. tighter coupling between Ojas Data and the authority's scope model.

**O-2.** Should `no-progress-halt-record` (§3.5) distinguish between *per-request* halts and *session-terminal* halts via the `terminal` boolean, or should they be separate record types? Current draft uses the boolean; separate types may be cleaner for downstream consumers.

**O-3.** `pattern-confidence-class-code` (§6.2) has three levels (exact / scope / actor match). Is `scope-match` (different actor, same scope) a useful signal, or does it belong to a different defense (e.g., insider-threat detection)?

**O-4.** Stream partitioning (§8.3) currently uses `tenant-id` as default partition key. For very large single-tenant deployments, a finer-grained key (e.g., `(tenant, entity)`) may be needed. Worth specifying tenancy classes here or leaving to deployment.

**O-5.** Should `reconciliation-batch-summary` (§7.2) carry the digests of the records compared, to enable forensic re-verification later? Storage cost vs. forensic value.

---

## Closing

This catalog is the v0.3 baseline. It supersedes the v0.2 CDDL encoding-specific schema. It is schema-language-neutral by design.

The next downstream artifact, per parent §10, is JSON Schema generation from this catalog. That step happens after review of this catalog completes.

Until O-1 through O-5 are decided and any other review feedback is incorporated, this catalog is **draft / review candidate**, not frozen.
